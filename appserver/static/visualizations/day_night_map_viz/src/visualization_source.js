...
function(
    ...
    var sun_map_viz = {
        initialize: function() {
            ...
            this.$svg = $("<svg class='day_night_map-svg'></svg>").appendTo(this.$container);
        ...
        updateView: function(data, config) {
            const map = new DaylightMap(this.$svg[0], new Date());
            map.init();

            updateDateTime(map.currDate);

            $(document).on('update-date-time', function(date){ updateDateTime(map.currDate); });
            ...
            $('.js-skip').on('click', function(e) {
                ...
                if ($el.attr('data-animate')) { animate = true; }
    ...
    */
    class DaylightMap {
        static initClass() {
            ...
            this.prototype.lineFunction = d3.svg.line().x(d => d.x).y(d => d.y).interpolate('basis');
        }
        ...
        getPathString(northSun) {
            let yStart;
            if (!northSun) { yStart = 0; } else { yStart = this.MAP_HEIGHT; }
        ...
        drawLand() {
            ...
            setTimeout(function(){
                const projection = d3Geo.equirectangular()
                    .scale(this.PROJECTION_SCALE)
                    .translate([this.MAP_WIDTH / 2, this.MAP_HEIGHT /2])
                    .precision(0.1);

                const worldPath = d3Geo.path().projection(projection);
        ...
        drawCities() {
            /*TODO return $.get(this.CITIES_DATA_URL,  data => {
